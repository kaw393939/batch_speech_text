...
def process_file(file: Path):
    ...
    try:
        ...
        final_output_path = OUTPUT_FOLDER / f"{file.stem}.mp3"
        ...
        processed_file_path = OUTPUT_FOLDER / f"{file.stem}.processed.txt"
        if processed_file_path.exists():
            processed_file_path = processed_file_path.with_name(f"{file.stem}_{int(time.time())}.processed.txt")
